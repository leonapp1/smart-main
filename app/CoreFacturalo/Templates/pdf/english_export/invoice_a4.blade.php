...
<body>
    ...
    <table class="full-width mt-5">
        <tr>
            ...
            <td width="120px">N. CTA DETRACTIONS</td>
            <td width="8px">:</td>
        ...
        @if ($document->detraction)
        <td width="140px">B/S SUBJECT TO DETRACTION</td>
        <td width="8px">:</td>
        ...
        <tr>
            ...
            @if ($document->detraction)
            <td width="120px">DETRACTION AMOUNT</td>
            <td width="8px">:</td>
        ...
        <tr>
            ...
            <td width="120px">PAY CONSTANCY</td>
            <td width="8px">:</td>
        ...
        <tr>
            <td colspan="4"><strong>DETAIL - CARGO TRANSPORTATION SERVICES</strong></td>
        </tr>
        <tr>
            <td class="align-top">ubigeo origin</td>
            <td>:</td>
            <td>{{ $document->detraction->origin_location_id[2] }}</td>

            <td width="120px">source address</td>
            <td width="8px">:</td>
            <td>{{ $document->detraction->origin_address }}</td>
        </tr>
        <tr>
            <td class="align-top">destination location</td>
            <td>:</td>
            <td>{{ $document->detraction->delivery_location_id[2] }}</td>

            <td width="120px">destination address</td>
            <td width="8px">:</td>
            <td>{{ $document->detraction->delivery_address }}</td>
        </tr>
        <tr>
            <td class="align-top" width="170px">Referential value of transport service</td>
            <td>:</td>
            <td>{{ $document->detraction->reference_value_service }}</td>

            <td width="170px">Effective load reference value</td>
            <td width="8px">:</td>
            <td>{{ $document->detraction->reference_value_effective_load }}</td>
        </tr>
        <tr>
            <td class="align-top">Payload referential value</td>
            <td>:</td>
            <td>{{ $document->detraction->reference_value_payload }}</td>

            <td width="120px">trip detail</td>
            <td width="8px">:</td>
    ...
    <table class="full-width mt-3">
        <tr>
            <td width="120px">P. ACCUMULATED</td>
            <td width="8px">:</td>
            <td>{{ $document->person->accumulated_points }}</td>

            <td width="140px">POINTS FOR THE PURCHASE</td>
            <td width="8px">:</td>
    ...
    <table class="full-width mt-3">
        ...
        <tr>
            <td width="120px">N° SEAT</td>
            <td width="8px">:</td>
            <td>{{ $transport->seat_number }}</td>
            <td width="120px">M. PASSENGER</td>
            <td width="8px">:</td>
        ...
        <tr>
            <td width="120px">U. ORIGIN</td>
            <td width="8px">:</td>
            <td>{{ $origin_district }}</td>
            <td width="120px">D. ORIGIN</td>
            <td width="8px">:</td>
            <td>{{ $transport->origin_address }}</td>
        </tr>
        <tr>
            <td width="120px">U. DESTINATION</td>
            <td width="8px">:</td>
            <td>{{ $destinatation_district }}</td>
            <td width="120px">D. DESTINATION</td>
            <td width="8px">:</td>
    ...
    <table class="full-width mt-3">
        ...
        <tr>
            ...
            <td width="120px">Deliver date</td>
            <td width="8px">:</td>
        ...
        <tr>
            <td width="120px">DOCUMENT AFFECTED</td>
            <td width="8px">:</td>
    ...
    <table class="full-width mt-10 mb-10">
        <thead class="">
            <tr class="bg-grey">
                <th class="border-top-bottom text-center py-2" width="9%">AMOUNT</th>
        ...
        <tbody>
            ...
            <tr>
                <td colspan="5" class="text-right font-bold">OP. EXONERED: {{ $document->currency_type->symbol }}</td>
            ...
            <tr>
                <td colspan="5" class="text-right">OP. TAXED: {{ $document->currency_type->symbol }}</td>
            ...
            <tr>
                <td colspan="5" class="text-right font-bold">{{(($document->total_prepayment > 0) ? 'ADVANCE':'FULL DISCOUNT')}}
            ...
            @endphp
            <tr>
                <td colspan="5" class="text-right font-bold">CHARGES ({{$total_factor}}
                    %): {{ $document->currency_type->symbol }}</td>
                <td class="text-right font-bold">{{ number_format($document->total_charge, 2) }}</td>
            </tr>
            @else
            <tr>
                <td colspan="5" class="text-right font-bold">CHARGES: {{ $document->currency_type->symbol }}</td>
                <td class="text-right font-bold">{{ number_format($document->total_charge, 2) }}</td>
            </tr>
            @endif
            @endif
            ...
            <tr>
                <td colspan="5" class="text-right font-bold">PERCEPTION: {{ $document->currency_type->symbol }}</td>
            ...
            <tr>
                <td colspan="8" class="text-right font-bold" style="font-size: 16px;">TOTAL: {{ $document->currency_type->symbol }}</td>
                <td class="text-right font-bold" style="font-size: 16px;">{{ number_format($document->total, 2) }}</td>
            </tr>
            <tr>
                <td colspan="5" class="text-right">TOTAL WITHHOLDING ({{ $document->retention->percentage * 100 }}
                    %): {{ $document->currency_type->symbol }}</td>
                <td class="text-right">{{ number_format($document->retention->amount, 2) }}</td>
            </tr>
            <tr>
                <td colspan="5" class="text-right">NET AMOUNT: {{ $document->currency_type->symbol }}</td>
            ...
            <tr>
                <td colspan="5" class="text-right font-bold">OUTSTANDING AMOUNT: {{ $document->currency_type->symbol }}</td>
                <td class="text-right font-bold">{{ number_format($document->total_pending_payment, 2) }}</td>
            </tr>
            @endif

            @if($balance < 0) <tr>
                <td colspan="5" class="text-right font-bold">TURNED: {{ $document->currency_type->symbol }}</td>
    ...
    <table class="full-width">
        <tr>
            ...
            <td width="35%" class="text-right">
                ...
                <p style="font-size: 9px">Code Hash: {{ $document->hash }}</p>
            </td>
        </tr>
    </table>
    @php
    ...
    <table class="full-width">
        <tr>
            <td>
                <strong>PAYMENT CONDITION: {{ $paymentCondition }} </strong>
            </td>
        </tr>
    </table>
